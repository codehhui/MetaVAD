...
class Channel_Embeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        return embeddings  # (8,64,64) (8,64,128) (8,64,256) (8,64,512)
...
class Attention_org(nn.Module):
    def __init__(self, vis, channel_num, KV_size=960, num_heads=4):
...
class Block_ViT(nn.Module):
    def __init__(self, vis, channel_num, expand_ratio=4, KV_size=960):
...
class ChannelTransformer(nn.Module):
    ...
    def forward(self, h0, h1, h2, h3):

        emb1 = self.embeddings_1(h0)
        emb2 = self.embeddings_2(h1)
        emb3 = self.embeddings_3(h2)
        emb4 = self.embeddings_4(h3)

        encoded1, encoded2, encoded3, encoded4, attn_weights = self.encoder(emb1,emb2,emb3,emb4)  # (B, n_patch, hidden)
        x1 = self.reconstruct_1(encoded1) if h0 is not None else None
        x2 = self.reconstruct_2(encoded2) if h1 is not None else None
        x3 = self.reconstruct_3(encoded3) if h2 is not None else None
        x4 = self.reconstruct_4(encoded4) if h3 is not None else None

        x1 = x1 + h0  if h0 is not None else None
        x2 = x2 + h1  if h1 is not None else None
        x3 = x3 + h2  if h2 is not None else None
        x4 = x4 + h3  if h3 is not None else None

        # x1 = x1.view(2, x1.size(1), x1.size(2), x1.size(3))
        # x2 = x1.view(2, x2.size(1), x2.size(2), x2.size(3))
        # x3 = x1.view(2, x3.size(1), x3.size(2), x3.size(3))
        # x4 = x1.view(2, x4.size(1), x4.size(2), x4.size(3))
...
if __name__ == "__main__":
    ...
    h0 = torch.randn(8, 64, 256, 256)
    h1 = torch.randn(8, 128, 128, 128)
    h2 = torch.randn(8, 256, 64, 64)
    h3 = torch.randn(8, 512, 32, 32)

    emb1 = torch.randn(8, 64, 64)
    emb2 = torch.randn(8, 64, 128)
    emb3 = torch.randn(8, 64, 256)
    emb4 = torch.randn(8, 64, 512)
